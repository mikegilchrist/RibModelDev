...
\item Fixed error for Pr(Elongation at position $j$) in Equation (\ref{eq:defEElongPrII}) by adding $\lambda_j v_{g,i}$ term.
  Additionally, applied fix to Equation (\ref{eq:defESigmag}) by adding missing term to product term.
...
When the waiting time for elongation each position $j$ is known, then
 \begin{align}
  \label{eq:defElongPr}
\Pr(\text{Elongation at position $j$}) &=\frac{\vj}{\wgj + \vj} \\
\intertext{and}
   \label{eq:defSigmag}
...
E\left[\Pr(\text{Elongation at position $j$})\right] &= \int_0^\infty \frac{\vj}{\wgj + \vj} f\left(\wgj | \alphaj, \lambdaj\right) d\wgj\\
...
E\left[\Pr(\text{Elongation at position $j$})\right] &= \exp\left[\lambdaj \vj\right] \left(\lambdaj \vj\right)^\alphaj  \Gamma(1-\alphaj, \lambdaj \vj)
\end{align}

Assuming independence in \wgi between positions means that the expected value of $\sigmag(i)$ is, 
\begin{align}
\label{eq:defESigmag}
  \Esigmagi &=  \exp\left[\sum_{j=1}^i \lambdaj \vj\right]  \prod_{j=1}^{i}\left(\lambdaj \vj  E_{\alphaj}\left(\lambdaj \vj\right) \right)
\end{align}
Note that these \lambdaj terms are equivalent to \lambdac (but not \lambdacprime, which is used below).

To approximate \Esigmagi, we set $\vj = 1/(c_j b)$ where $c_j$ is a codon specific scaling factor $>0$ and $b$ is the mean nonsense error rate and taking a first order Taylor Series approximation around $b=0$.
\begin{align}
\label{eq:approxESigmag}
  \Esigmagi &\approx 1 - \sum_{j=1}^i \frac{\alphaj}{\lambdaj \vj}+ O(\bar{1/v})
\end{align}
...
Note also that $\Ygi!$ can be dropped from any Likelihood calculations.
