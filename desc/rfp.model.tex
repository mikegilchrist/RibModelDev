...
E\left[\Pr(\text{Elongation at position $j$})\right] &= \exp\left[\lambdaj \vgj\right] \left(\lambdaj \vgj\right)^\alphaj  \Gamma(1-\alphaj, \lambdaj \vgj)
\end{align}
Given the complexity of this result, it may be worthwhile to explore Taylor Series approximations around $1/\vgj = 0$ or $\vgj = c_j b$ where $c_j$ is a codon specific scaling factor $>0$ and $b$ is the mean nonsense error rate. . 

Assuming independence in \wgi between positions means that the expected value of $\sigmag(i)$ is, 
\begin{align}
\label{eq:defESigmag}
  \Esigmagi &=  \exp\left[\sum_{j=1}^i \lambdaj \vgj\right]  \prod_{j=1}^{i}\left(\lambdaj \vgj  E_{\alphaj}\left(\lambdaj \vgj\right) \right)
\end{align}
