...
\item Fixed error for Pr(Elongation at position $j$) in Equation (\ref{eq:defElongPrII}) by adding $\lambda_j v_{g,i}$ term.
  Additionally, applied fix to Equation (\ref{eq:defSigmag}) by adding missing term to product term.
...
\Pr(\text{Elongation at position $j$}) &= \int_0^\infty \frac{\vgj}{\wgj + \vgj} f\left(\wgj | \alphaj, \lambdaj\right) d\wgj\\
 \label{eq:defElongPrII}
 &= \lambdaj \vgj \exp\left[\lambdaj \vgj\right] E_{p = \alphaj}\left(\lambdaj \vgj\right)
\end{align}
...
\begin{align}
  \label{eq:defSigmag}
  \sigmag(i) &=  \exp\left[\sum_{j=1}^i \lambdaj \vgj\right]  \prod_{j=1}^{i} \lambdaj \vgj  E_{\alphaj}\left(\lambdaj \vgj\right)
\end{align}
...
First, we generate $W_i$  from $\text{Gamma}(\alphac, \lambdacprime)$ and then we generate $\Ygi$ from $\text{Poisson}\left(\psi_g \, \sigmag(i-1)\, W_i\right)$ where here $\sigmag(i) = \prod_{j=1}^i v_j/\left(v_j + W_j\right)$.
We use this form instead of the result in Eq.~(\ref{eq:defSigmag}), because when simulating data, $W_j$ is known and, as a result, integration is unnecessary.
