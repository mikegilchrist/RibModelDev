...
genome.file <- "simulatedFONSE_phiScaled10000_b-0.1.fasta"
phi.file <- "simFONSE_scaled10000.phi.csv" 
mut.file <- "S.cer.mut.ref.csv"
sel.file <- "deltaomega_b-0.1.csv"

run.name <- "simulatedFONSE_b-0.1_scaled10000"
...
parameter <- initializeParameterObject(genome, sphi_init, numMixtures, geneAssignment, model="FONSE", split.serine = TRUE, mixture.definition = mixDef)
...
setRestartSettings(mcmc, paste(run.name, "restartFile.rst", sep=""), 100, TRUE)
